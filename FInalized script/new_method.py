...
def detect_object_IR(INPUT_IMAGE,CANVAS_IMAGE):
    ...
    return CANVAS_IMAGE, box_locations
...
def enhance_greyscale(greyscale_img, temp_img, gradient_scale=50):

   
    # Normalize the greyscale image to have a mean of 128
    grey_norm = greyscale_img.astype(np.float32)
    grey_norm -= np.mean(grey_norm)
    grey_norm += 128
    grey_norm = np.clip(grey_norm, 0, 255).astype(np.uint8)

   
    temp_img = temp_img.astype(np.float32)  # Ensure it's float for precision
    grad_x = cv2.Sobel(temp_img, cv2.CV_32F, 1, 0, ksize=3)
    grad_y = cv2.Sobel(temp_img, cv2.CV_32F, 0, 1, ksize=3)
    ...
    enhanced_img = grey_norm - temp_gradient
