...
IR.set(cv2.CAP_PROP_FRAME_HEIGHT, 512) 
IR.set(cv2.CAP_PROP_FRAME_WIDTH, 640) 
...
def send_target_location():
	...
	print(f"Finding target with camera at lat={current_lat} lon={current_lon}")
...
try:
	with open(csv_filename,'w',newline='') as csv_file:
		...
		while True:
			#capture frames with current lat and lon
			ir_stream_true, frame1 = IR.read()
			rgb_stream_true, frame2 = RGB.read()
			image_latitude = current_lat
			image_longitude = current_lon
			
			if ir_stream_true and rgb_stream_true:

				grey_scale_image = cv2.normalize(frame1, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
				cropped_rgb = track.homographic_transform(grey_scale_image,frame2)
				uncropped_rgb = frame2
				#grey_scale_image1 =track.enhance_greyscale(grey_scale_image,frame1)
				#cv2.imshow('enhanced', grey_scale_image1)
			
				grey_scale_image_fin,returned_boxes, centers , colored_image  = track.detect_object_IR(grey_scale_image ,frame1, grey_scale_image)
				clean.show_screens(cropped_rgb,colored_image)
				corners = track.find_corners(returned_boxes)
				objectLocations = convert.returnSpatialLocation(corners, grey_scale_image_fin)
				...
				if num%5 == 0: #set interval between saved frames here
					...
					cv2.imwrite(final_image_ir,colored_image)
					cv2.imwrite(final_image_rgb,cropped_rgb)
					cv2.imwrite(final_temperature_frame,frame1)
					
					if image_latitude < 0:
						refrence_lattitude = 'S'
					else:
						refrence_lattitude = 'N'
					if image_longitude < 0:
						refrence_longitude = 'W'
					else:
						refrence_longitude = 'E'
						
					writer.writerow([np.abs(current_lat),
									np.abs(current_lon),
									st,
									refrence_lattitude,
									refrence_longitude,
