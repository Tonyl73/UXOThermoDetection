	...
	with open(csv_filename,'w',newline='') as csv_file:
		...
		while True:#record indefinitely (until user presses q), replace with "while True"
			...
			if ir_stream_ret and rgb_stream_ret:
				...
				grey_scale_image3,BOXES = track.detect_object_IR(grey_scale_image2,grey_scale_image)
				...
				if num%10 == 0: #set interval between saved frames here
					...
					if coords.lon < 0:
						...
						ref_lon = 'E'
					writer.writerow([np.abs(coords.lat),np.abs(coords.lon),st,ref_lat,ref_lon])
					image_num += 1
					time.sleep(.01)   
				num += 1
